    ...
    <div className="w-full mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <h3 className="text-xl font-bold text-slate-900 dark:text-white">{t("noResults")}</h3>
          <p className="mt-2 text-slate-500 dark:text-slate-400 max-w-sm mx-auto">
            {t("noResultsSummary")}
          </p>
          <Button variant="outline" className="mt-6 rounded-2xl dark:border-slate-800" onClick={() => setFilters({ search: "" })}>
            {t("clearSearch")}
          </Button>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {patients.map((patient) => (
            <PatientCard key={patient.id} patient={patient} onClick={(id) => router.push({ pathname: "/patients/[id]", params: { id } })} />
          ))}
        </div>
      )}
    </div>
