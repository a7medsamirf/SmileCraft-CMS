      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            {new Intl.DateTimeFormat(locale === "ar" ? "ar-EG" : "en-US", { weekday: "long", day: "numeric", month: "long" }).format(selectedDate)}
          </p>
        </div>
        <div className="flex items-center gap-2 rounded-full border border-blue-100 bg-blue-50 px-3 py-1 text-sm font-semibold text-blue-600 dark:border-blue-900 dark:bg-blue-900/30 dark:text-blue-400">
          ...
          <span>{appointments.length} {t("appointmentsCount") || "مواعيد"}</span>
        </div>
      </div>
          ...
          <table className="w-full whitespace-nowrap">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {appointments.map((apt) => (
                <tr key={apt.id} className="transition-colors hover:bg-slate-50/50 dark:hover:bg-slate-800/50 group">
                  ...
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-3 justify-start">
                      ...
                      <span className="font-medium text-slate-700 dark:text-slate-200">{apt.patientName}</span>
                    </div>
                  </td>
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-2 text-slate-600 dark:text-slate-400 justify-start">
                      <Activity className="h-4 w-4" />
                      {apt.procedure}
                    </div>
                  </td>
                  <td className="px-4 py-4 text-right">
                    <Badge variant={statusVariantMap[apt.status].variant} className="px-3 py-1">
                      {t(`status${apt.status.charAt(0) + apt.status.slice(1).toLowerCase().replace(/_/g, "")}`)}
                    </Badge>
                  </td>
                </tr>
              ))}
